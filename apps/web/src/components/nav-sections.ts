/**
 * Definición centralizada de las secciones de navegación lateral del HIS.
 *
 * Extraído de app-shell.tsx (Tarea 2a — rediseño v2.0) para permitir:
 *   1) Reutilización en app-sidebar.tsx (Shadcn sidebar).
 *   2) Tests unitarios de visibilidad sin montar React.
 *   3) Futura generación server-side de breadcrumbs.
 *
 * Nivel A scope: los items con `requiredServiceUnits` se filtran por el
 * predicado `isItemVisible` de nav-visibility.ts.
 */
...
// Importamos los iconos de lucide-react. Al ser un archivo .ts (no .tsx) no
// necesitamos React — los iconos son solo referencias de componente.
import {
  LayoutDashboard,
  Users,
  Bed,
