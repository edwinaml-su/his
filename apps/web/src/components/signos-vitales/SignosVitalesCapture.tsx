...
/**
 * Captura de signos vitales — módulo transversal (CC-0012, mockup avante7).
 *
 ...
 *     son obligatorios; `showErrors` los resalta al intentar guardar.
 ...
 *     cintura + ICT), balance hídrico, gineco-obstétrico y EVA.
 *   - Gineco-obstétrico solo si sexo femenino; FPP solo si además está en
 *     edad fértil (puedeEmbarazo).
 *
 * Base de código: `apps/web/src/app/(clinical)/ece/evolucion/nueva/_components/
 * SignosVitalesCapture.tsx` (CC-0006), generalizado para reutilizar fuera de
 * evolución: recibe `paciente {sexo, edad}` por props (ya lo hacía) y además
 * controla `fppActivo` como parte del estado persistido (CC-0012 sí guarda el
 * estado del interruptor; evolución no lo hacía por diseño).
 *
 * Rangos/alertas/cálculos viven en `@his/contracts` (packages/contracts/src/
 * validators/signos-vitales.ts) — misma fuente que trpc y evolución.
 */
...
import { tieneSignosForm, type VitalesFormState } from "./types";
...
export interface SignosVitalesCaptureProps {
  idPrefix?: string;
  ...
  edad?: number | null;
  /** Fuerza el resaltado de errores (al intentar guardar, R4.1). */
  showErrors?: boolean;
}
...
export function SignosVitalesCapture({
  ...
  function setField(field: keyof VitalesFormState, val: string | number | boolean) {
  ...
  const fpp = mostrarFpp && value.fppActivo ? fppNaegele(value.fechaUltimaRegla) : null;
  const gestacion = mostrarFpp && value.fppActivo ? gestacionDesdeFur(value.fechaUltimaRegla) : null;

  const mostrarGineco = esFemenino(sexo);
  const hayDatos = tieneSignosForm(value);

  return (
    <div className="space-y-4" data-testid="signos-vitales-capture">
      {/* Fila de alertas: 3 estados recalculados en vivo. */}
      <div
              ...
              {mostrarFpp && (
                <div className="space-y-1">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <Switch
                      checked={value.fppActivo}
                      onCheckedChange={(checked) => setField("fppActivo", checked)}
