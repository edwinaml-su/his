 ...
 * laboratorio (CC-0013). Mismo patrón que
 * `(clinical)/ece/historia-clinica/nueva/_components/selector-cuenta.tsx`
 * (CC-0007) — se muestra cuando se entra a `/lis/orders/new` sin
 * `?cuentaId=`: busca paciente, elige una de sus cuentas, navega a
 * `?cuentaId=<uuid>`.
 */
...
export function SelectorCuenta({ onSelect }: SelectorCuentaProps) {
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-10">
      <h1 className="text-lg font-semibold text-foreground">Nueva orden de laboratorio</h1>
      <p className="mt-1 text-sm text-muted-foreground">
        Seleccione la cuenta del paciente para escoger los exámenes a solicitar.
      </p>
        ...
        <div className="mt-5">
          ...
          <div className="mt-3 overflow-hidden rounded-lg border border-border">
            {cuentasQ.isLoading ? (
              ...
              <p className="px-4 py-3 text-sm text-muted-foreground">
                Este paciente no tiene cuentas. Cree una cuenta desde el expediente antes de
                solicitar exámenes.
              </p>
            ) : (
              <ul className="divide-y divide-border">
