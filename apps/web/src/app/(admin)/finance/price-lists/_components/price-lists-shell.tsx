 ...
 *
 * Parametrización admin (2026-09-10): la lista DEFAULT del resolver de
 * precios (SQL 228, `ServicePriceList.isDefault`) se marcaba por SQL
 * directo — ahora es un toggle ADMIN/DIR (`servicePriceList.setDefault`).
 * `roleCodes` se resuelve server-side en `page.tsx` (mismo patrón que
 * `(clinical)/lis/orders/new` / `finance/invoices/nuevo`).
 */
...
const ROLES_LISTA_DEFAULT = ["ADMIN", "DIR"];
...
export function PriceListsShell({ roleCodes }: { roleCodes: string[] }) {
  const [activeFilter, setActiveFilter] = React.useState("activos");
  const canSetDefault = roleCodes.some((r) => ROLES_LISTA_DEFAULT.includes(r));
  ...
  const setDefault = trpcAny.servicePriceList.setDefault.useMutation({
    onSuccess: () => query.refetch(),
  });

  function handleSetDefault(row: PriceListRow) {
    if (
      !window.confirm(
        `¿Marcar "${row.name}" como lista por defecto? Se usará como fallback de precios para TODA la organización cuando una cuenta no tenga tarifario asignado o el asignado no resuelva el código.`,
      )
    ) {
      return;
    }
    setDefault.mutate({ priceListId: row.id, isDefault: true });
  }
                  ...
                  <TableRow key={row.id}>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        {row.isDefault ? (
                          <Badge variant="default" title="Fallback de precios de toda la organización">
                            Por defecto
                          </Badge>
                        ) : null}
                      </div>
                    ...
                    <TableCell className="text-right">
                      <div className="inline-flex flex-wrap justify-end gap-2">
                        ...
                        {canSetDefault && row.active && !row.isDefault ? (
                          <Button
                            size="sm"
                            variant="outline"
