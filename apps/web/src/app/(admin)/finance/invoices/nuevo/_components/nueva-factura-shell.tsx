 ...
 *
 * docs/48 Ola 4b (H-17) — override de precio: `invoice.create` rechaza con
 * PRECONDITION_FAILED cuando un precio no se pudo resolver o no coincide con
 * el resuelto server-side (docs/48 Ola 2 C2-3/H-03), a menos que el ítem
 * incluya `overridePrecio.justificacion` Y el usuario tenga rol ADMIN/DIR
 * (`ROLES_OVERRIDE_PRECIO` en invoice.router.ts). Este componente detecta ese
 * error, y si `roleCodes` (resuelto server-side por `page.tsx`, mismo patrón
 * que `(clinical)/lis/orders/new`) incluye ADMIN/DIR, abre un dialog para
 * capturar la justificación y reintenta el mismo submit con
 * `overridePrecio` en cada línea (el backend sólo lo usa en las líneas que
 * no coincidieron; en las que sí coincidieron lo ignora).
 */
...
import { attachOverrideJustificacion, esErrorDePrecio, puedeOverridePrecio } from "./invoice-override";
...
export interface NuevaFacturaShellProps {
  /** Roles efectivos del tenant, resueltos server-side por `page.tsx`. */
  roleCodes: string[];
}

export function NuevaFacturaShell({ roleCodes }: NuevaFacturaShellProps) {
  const router = useRouter();
  const utils = trpcAny.useUtils();

  const puedeForzarPrecio = puedeOverridePrecio(roleCodes);
  ...
  const [overrideJustificacionError, setOverrideJustificacionError] = React.useState<string | null>(null);
  const [pendingStatus, setPendingStatus] = React.useState<"DRAFT" | "ISSUED" | null>(null);
  ...
  const createMutation = trpcAny.invoice.create.useMutation({
    ...
    onError: (err: { message: string }) => {
      // docs/48 Ola 4b (H-17) — mismatch/no-resoluble de precio con rol
      // habilitado para override: ofrecer el dialog en vez de solo mostrar
      // el error. Sin el rol, o si el error es de otra naturaleza, cae al
      // comportamiento previo (mensaje inline).
      if (puedeForzarPrecio && esErrorDePrecio(err.message)) {
        setSaving(false);
        setOverrideJustificacionError(null);
  ...
  /**
   * docs/48 Ola 4b (H-17) — `justificacion` (si viene) se adjunta a TODAS las
   * líneas: el servidor rechaza el submit completo en la PRIMERA línea que no
   * coincide (`invoice.router.ts create`, loop secuencial), así que el
   * cliente no puede saber de antemano cuál línea específica falló. Adjuntar
   * `overridePrecio` a todas es inocuo — el servidor solo lo lee en las
   * líneas donde `unitPrice` no coincide con lo resuelto; en las que sí
   * coinciden, lo ignora.
   */
  function buildPayload(status: "DRAFT" | "ISSUED", justificacion?: string) {
    return {
      patientId: patientId.trim(),
      ...(insurerId ? { insurerId } : {}),
      ...
      currencyId,
      status,
      items: attachOverrideJustificacion(
        items.map((it) => ({
  ...
  function handleOverrideConfirm() {
    if (overrideJustificacion.trim().length < 10) {
      setOverrideJustificacionError("La justificación debe tener al menos 10 caracteres.");
      return;
    }
    ...
    setSaving(true);
    createMutation.mutate(buildPayload(pendingStatus, overrideJustificacion.trim()));
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* docs/48 Ola 4b (H-17) — dialog de override de precio (solo ADMIN/DIR llega aquí). */}
      <Dialog open={overrideDialogOpen} onOpenChange={setOverrideDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Forzar precio (override)</DialogTitle>
            <DialogDescription>
              El precio de al menos una línea no se pudo resolver o no coincide con el
              tarifario. Ingresa la justificación para forzarlo — quedará auditada en la
              factura.
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-1">
            <Label htmlFor="override-justificacion">
            ...
            <Input
              ...
              placeholder="Ejemplo: tarifa pactada fuera de lista, autorizada por Dirección"
              aria-invalid={Boolean(overrideJustificacionError)}
              autoFocus
            />
            {overrideJustificacionError ? (
              <p className="text-sm text-destructive">{overrideJustificacionError}</p>
            ) : null}
          </div>
          ...
          <DialogFooter>
            ...
            <Button type="button" onClick={handleOverrideConfirm} disabled={saving}>
              Forzar precio y guardar
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
