...
- Trunk-based: PRs cortos contra `main`. **Branch protection ACTIVA** (desde 2026-08-18) con 3 required status checks: `Build, Lint, Test, Typecheck` · `Build & push (GHCR)` · `axe-core WCAG 2.1 AA — 5 páginas baseline`. **`enforce_admins: true` desde 2026-09-12** — NADIE (ni admin, ni `--admin`) puede mergear con required checks pendientes o rojos; la razón: 6 merges con head desactualizado perdieron commits (#606, #613, #617, #619, #631, #635). `strict: false` → no exige rama al día con `main`. `Playwright Smoke (@smoke)` **no** es requerido a propósito (ver §E2E). Verificá el estado real con `gh api repos/edwinaml-su/his/branches/main/protection` antes de asumir que algo bloquea: entre 2026-06-30 y 2026-08-18 la protección existía pero `contexts` estaba **vacío**, o sea que se podía mergear con el CI en rojo.
- **Guardas locales** (`.claude/settings.json` + `.claude/hooks/*.mjs`, desde 2026-09-12): `guard-merge` bloquea `gh pr merge` si los required checks no están verdes o si el head remoto ≠ rama local (anti stale-head); `guard-sql-reapply` bloquea `apply_migration` de SQLs numerados ya marcados "NO re-aplicar"; `warn-payloads` recuerda la regla de merge secuencial al tocar `payloads.ts`. Skill `pre-pr-review` (`.claude/skills/`): revisión con contexto fresco + checklist de lecciones pagadas ANTES de cada `gh pr create`.
