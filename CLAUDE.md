...
- **Rate limit en endpoints auth (OWASP A07):** patrón in-memory `checkRateLimit`/`recordAuthAttempt` en `packages/trpc/src/lib/` cableado en `firma.requestRecovery` + `mfa.verify`. Endpoints auth nuevos deben pasar por este guard (lanza `TOO_MANY_REQUESTS`).
- **Reset de password admin:** `userAdmin.resetPassword` usa `hashPin` (argon2id) de `@his/infrastructure`, cierra credenciales PASSWORD vigentes + crea nueva en tx, bloquea auto-reset, exige rol ADMIN + razón auditada. No toca Supabase Auth (OIDC sigue su flujo).
