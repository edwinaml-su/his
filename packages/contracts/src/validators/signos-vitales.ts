 ...
 * signos vitales (CC-0012 — módulo transversal, mockup avante7).
 *
 * Origen: extraído/consolidado desde `apps/web/src/lib/evolucion/signos-vitales.ts`
 * (CC-0006), que ahora re-exporta este módulo para no duplicar lógica entre
 * web y trpc. Los nombres de campo son en español para integración directa
 * con `SignosState` (evolución) y el módulo compartido `apps/web/src/components/signos-vitales`.
 * Independiente de `evaluateVitalAlerts` (@his/contracts/schemas/inpatient) que
 * usa nombres en inglés y corre en otra capa (censo hospitalario legacy).
 */
...
// ─── Rango de edad fértil (parametrizable) ───────────────────────────────────

export const RANGO_EDAD_FERTIL = { min: 10, max: 55 } as const;

// ─── Rangos de validación (mockup avante7 — vitalsBodyHTML) ─────────────────
...
/** Clasificación del ICT. El componente mapea `key` → color del DS. */
...
/** "Puede estar embarazada" = femenino y en edad fértil. */
 ...
 * Umbrales exactos del mockup avante7 (computeAlerts, L1066-1080).
 */
export function computeAlertasVitales(v: VitalesInput): string[] {
  ...
  // Oliguria: diuresis < 0.5 mL/kg/h (usando el peso en kg)
...
/** Etiqueta textual de la escala EVA (0–10), 5 bandas (mockup avante7). */
