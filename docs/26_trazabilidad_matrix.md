...
## 9. Remediación E2E 2026-06-10

Hallazgos de la sesión E2E (`docs/uat/e2e-2026-06-10/`) y su remediación.

| ID | Sev. | Hallazgo | Fix (evidencia) | Tests | Estado |
|---|---|---|---|---|---|
| INC-2026-06-10-001 | P0 | 500 global por agotamiento del pool Supabase (session mode, 15 conexiones); el layout `async` lanza sin error boundary → toda la app cae. | **Código:** `apps/web/src/app/error.tsx` (captura el throw de los layouts `(clinical)`/`(admin)` — boundary del segmento raíz), `apps/web/src/app/(clinical)/error.tsx`, `apps/web/src/app/(admin)/error.tsx`, `apps/web/src/components/error-fallback.tsx`. **Config (Edwin):** `DATABASE_URL` → transaction mode (pooler 6543 `?pgbouncer=true&connection_limit=1`) en Vercel; `schema.prisma` ya declara `directUrl` → zero-code. | typecheck @his/web ✅ · lint ✅ | ⚠️ código mergeable; falta env (Edwin) + prueba de carga ≥50 navegaciones + UAT |
| HJ-QX-001 | P2 | `bridge-cirugia.listProgramacionDia` JOIN por `pc.orden_id` (columna inexistente, PG 42703) rompe la tarjeta "Cirugías del día". Rama `salaQxId` además unía paciente por `p.id = r.episodio_id` (0 filas). | `packages/trpc/src/routers/ece/bridge-cirugia.router.ts` (JOIN → `pc.episodio_hospitalario_id = r.episodio_id` en ambas ramas; rama `salaQxId` alineada a `else`). Paridad `schema.prisma` `EcePreopChecklist` reconciliada con `sql/67_preop_checklist.sql`. | `bridge-cirugia.router.test.ts` test #10 (regresión SQL) ✅ · `ece-drift-models.test.ts` ✅ · typecheck @his/trpc + @his/database ✅ | ✅ tests verdes |
| HJ-QX-002 | P2 | Investigación de HJ-QX-001 (via Supabase MCP) reveló que **todo el bridge de quirófano** está roto contra el DDL actual (escrito vs esquema anterior): `programarCirugia` y `cancelarPrograma` con ~12 desajustes (orden_ingreso.instancia_id NOT NULL omitido; valores inválidos de estado_registro/modalidad/procedencia/servicio_categoria/estado; enum episodio; columnas inexistentes fecha_hora_fin/motivo_cancelacion/servicio_ingreso_id; personal por auth_user_id vs his_user_id; guard sobre estado_registro; nombre de paciente en `ece.paciente` que no tiene columna de nombre). Latente: 0 filas en prod (nunca corrió). | Reescritura de `bridge-cirugia.router.ts` espejando el canónico `orden-ingreso.router.ts`: instancia-first (ORD_ING/PREOP_CHECK), valores válidos, `his_user_id`, guard por estado de reserva, `fecha_hora_cierre`, nombre vía `public."Patient"`. | 11 tests ✅ · typecheck ✅ · **read path validado en vivo (MCP, 0 filas sin error)** · **write path validado por dry-run transaccional BEGIN…ROLLBACK con FKs reales** (programar+cancelar ejecutan limpio) | ✅ código validado contra prod |
| OBS-QX-DATA | Bajo | Datos maestros ECE de quirófano vacíos en prod: `ece.personal_salud`=0, `ece.sala_qx`=0. `programarCirugia` daría `PRECONDITION_FAILED` (sin personal) — error elegante, no crash. | Prerequisito de go-live: sembrar personal clínico ECE + quirófanos. Fuera del alcance del fix de código. | — | ⏳ pendiente (carga de catálogos) |
| OBS-BEDS | Bajo | `/beds` quedó en "Cargando…" en la ventana del P0. | Verificación: `apps/web/src/app/(clinical)/beds/page.tsx` maneja `isLoading`/`error`/`data` correctamente — sin bug determinista; síntoma de la query `eceCama.mapCompleto` colgada durante la saturación. | — | ✅ sin acción (reverificar tras FIX 1a) |
...
*Version 1.3: @Dev — 2026-06-10 — Seccion 9: remediacion E2E 2026-06-10 (INC-2026-06-10-001 error boundaries + HJ-QX-001 schema drift quirofano). Pendiente: env transaction mode (Edwin) + prueba de carga.*
