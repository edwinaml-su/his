...
| `BiomedicalEquipment` | ~~`giai_code`, `gln_ubicacion_actual`~~ / ~~`giaiCode`, `glnUbicacionActual` (camelCase, duplicadas)~~ | `82_equipment_gs1_extension.sql` (snake_case) + drift sin archivo (camelCase) | **RESUELTO por CC-0029** (`236_cc0029_giai_activos_biomedicos.sql`, 2026-09-14): se dropeó el par snake_case (0 filas, cascada de sus constraints/índices) y se completó el par camelCase huérfano (CHECK de formato + FK deferrable que le faltaban) como columna canónica — es la convención del resto de la tabla, a diferencia de lo que sugería esta fila antes de la resolución. `schema.prisma` sincronizado (`giaiCode String? @unique`, `glnUbicacionActual String?`); los `as any` de `services-equipment.router.ts` fueron eliminados. Ver `docs/CC/0029_giai_activos_biomedicos.md`. |
